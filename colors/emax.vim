if !has("gui_running")
	set background&
	finish
endif

set background=dark

hi clear
if exists("syntax_on")
    syntax reset
endif

let colors_name = "emax"

" hi CursorLine guibg=#191f21

" Default Colors
hi Normal guifg=#aaada6 guibg=#2e3436
hi NonText guifg=#2c3032 guibg=#2c3032 gui=none
hi Cursor guibg=#aaada6
hi ICursor guibg=#aaada6

" Search
hi Search guifg=#2e3436 guibg=#dc8f1e
hi IncSearch guibg=#2e3436 guifg=#dc8f1e

" Window Elements
hi StatusLine guifg=#2e3436 guibg=#aaada6 gui=none
hi StatusLineNC guifg=#2e3436 guibg=#888a85 gui=none
hi VertSplit guifg=#555753 guibg=#888a85 gui=none
hi Visual guibg=#000000
hi MoreMsg guifg=#729fcf
hi Question guifg=#6ac214 gui=none
hi WildMenu guifg=#eeeeec guibg=#0e1416
hi LineNr guifg=#3f4b4d guibg=#000000
hi SignColumn guibg=#2e3436

" Pmenu
hi Pmenu guibg=#2e3436 guifg=#eeeeec
hi PmenuSel guibg=#ffffff guifg=#2e3436
hi PmenuSbar guibg=#555753
hi PmenuThumb guifg=#ffffff

" Diff
hi DiffDelete guifg=#2e3436 guibg=#0e1416
hi DiffAdd guibg=#1f2b2d
hi DiffChange guibg=#2e3436
hi DiffText guibg=#000000 gui=none

" Folds
hi Folded guifg=#d3d7cf guibg=#204a87
hi FoldColumn guifg=#3465a4 guibg=#000000

" Specials
hi Title guifg=#dc8f1e
hi Todo guifg=#2e3436 guibg=#ccbc1e
hi SpecialKey guifg=#ef2929

" Tabs
hi TabLine guibg=#0a1012 guifg=#888a85
hi TabLineFill guifg=#0a1012
hi TabLineSel guibg=#555753 guifg=#eeeeec gui=none

" Matches
hi MatchParen guifg=#2e3436 guibg=#dc8f1e

" Tree
hi Directory guifg=#ccbc1e

" Syntax
" Constant #6ac214
hi Comment guifg=#656763
hi Constant guifg=#62bc14
hi Statement guifg=#729fcf gui=none
hi PreProc guifg=#dc8f1e
hi Identifier guifg=#d87070
hi Type guifg=#729fcf gui=italic
hi Special guifg=#888a85
hi Error guifg=#e0e0e0 guibg=#cc3333

hi Class guifg=#729fcf gui=italic
hi Structure guifg=#729fcf
hi DefinedName guifg=#dc8f1e
hi EnumeratorName guifg=#729fcf
hi EnumerationValue guifg=#ccbc1e
hi Member guifg=#aaada6
